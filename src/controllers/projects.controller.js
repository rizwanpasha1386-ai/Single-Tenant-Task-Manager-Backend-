...
async function addMembers(req,res) {
    try {
        ...
        const project=await PROJECT.findOneAndUpdate({_id:id,createdBy:req.user._id},
            {
                $addToSet: {
                    members: { $each: members } // 🔥 add multiple safely
                }
            },
            { new:true}
        )
        ...
        return res.status(200).json({msg:"Added members"})
    } catch (error) {
        return res.status(500).json({msg:"Server error",
            err:error.message
        })
...
module.exports={createProject,getAllProjects,getProjectById,updateProject,deleteProject,addMembers}